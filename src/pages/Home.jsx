    ...
    <div className="flex flex-col items-center justify-start mt-[-50px] h-auto">
      ...
      <div className="w-full flex flex-col items-center justify-center mt-7 relative z-10 gap-5 px-6 max-w-[440px] mx-auto">
        <div className="grid grid-cols-2 gap-3 w-full">
          ...
          <button className={`btn rounded-xl bg-[#ee0e9850] w-full h-[70px] lg:text-[20px] xs:text-[13px] font-medium sm:text-[19px] ${!user ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!user}>แบบทดสอบถาม Pre-Test</button>
          </a>
         <a href={user ? "https://docs.google.com/forms/d/e/1FAIpQLSeBhbm5PymqpM290yE0oMxX96Lbok2Gl4IkYpOKc_v_eaXHug/viewform" : "#"} target={user ? "_blank" : "_self"}>
          <button className={`btn rounded-xl bg-[#e0569065] w-full h-[70px] lg:text-[20px] xs:text-[12px] font-medium sm:text-[19px] ${!user ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!user}>แบบทดสอบถาม Post-Test</button>
          </a>
          </div>
